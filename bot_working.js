...
client.on('message', (message) => { 
    ...
    //Annoy people now its Gater
    if (command === "annoy") {
        client.users.fetch('377168665155600384').then(user => { 
            user.send('SURPRISE!');
        });
    }
...
//Position maker
    ...
    if (command === 'reset' && message.channel.id === CHANNEL_ID && message.member != null && message.member.hasPermission('MANAGE_ROLES')) {
        ...
        message.channel.send('**Champions League (' + originalNames.length + '**' + ' **/** ' + '**' + MAX_PLAYER + ')**' + ' **|** ' + tempLobby.join('/'));
    ...
    if (command === 'j' && message.channel.id === CHANNEL_ID) {
        if (!originalNames.includes(message.author.username) && !lobbyId.includes(message.author.id) && originalNames.length !== MAX_PLAYER) {
            originalNames.push(message.author.username);
            lobbyId.push(message.author.id);
            console.log(message.author.username + ' joined the lobby.');
            tempLobby.push(nameGenerator(message.author.username));
            unpicked.push(nameGenerator(message.author.username));
            if (originalNames.length !== MAX_PLAYER) {
                message.channel.send('**Champions League (' + originalNames.length + '**' + ' **/** ' + '**' + MAX_PLAYER + ')**' + ' **|** ' + tempLobby.join('/'));
            } else {
                pickPrint(red_team, blue_team, unpicked, cap, ret, bc, sup);
                message.channel.send(pickEmbed);
            }
        } else if (originalNames.includes(message.author.username) && lobbyId.includes(message.author.id)) {
            message.channel.send({embed: {
                color: 0xff0000,
    ...
    if (command === 'l' && message.channel.id === CHANNEL_ID) {
        if (originalNames.includes(message.author.username) && lobbyId.includes(message.author.id)) {
            originalNames = originalNames.filter(e => e != message.author.username);
            lobbyId = lobbyId.filter(e => e != message.author.id);
            console.log(message.author.username + ' left the lobby.');
            tempLobby = tempLobby.filter(e => e != nameGenerator(message.author.username));
    ...
    if (command === 'remove' && message.member != null && message.member.hasPermission('MANAGE_ROLES') && message.channel.id === CHANNEL_ID) {
        if (message.mentions.users.size) {
            ...
            if (originalNames.includes(taggedUsername.username) && lobbyId.includes(taggedUsername.id)) {
                originalNames = originalNames.filter(e => e != taggedUsername.username);
                lobbyId = lobbyId.filter(e => e != taggedUsername.id);
                console.log(taggedUsername.username + ' removed from the lobby.');
                tempLobby = tempLobby.filter(e => e != nameGenerator(taggedUsername.username));
    ...
    if (command === 'add' && message.member != null && message.member.hasPermission('MANAGE_ROLES') && message.channel.id === CHANNEL_ID) {
        if (message.mentions.users.size) {
            const taggedUsername = message.mentions.users.first();
            if (!originalNames.includes(taggedUsername.username) && !lobbyId.includes(taggedUsername.id) && originalNames.length !== MAX_PLAYER) {
                originalNames.push(taggedUsername.username);
                lobbyId.push(taggedUsername.id);
                console.log(taggedUsername.username + ' added to the lobby.');
                tempLobby.push(nameGenerator(taggedUsername.username));
